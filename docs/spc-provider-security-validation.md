...
| Meta / WhatsApp Cloud API | Private test-recipient phone number plus OTP delivery and verification metadata for the inactive `MFA_TEST` account | The `spc_mfa_test_code` Authentication template is approved in `en_US` and the pilot uses a dedicated Meta test sender from the same WhatsApp Business Account; the ADMIN-only pilot stores a keyed OTP hash, masks the destination, expires challenges after five minutes and does not change SPC login | Partially verified | Approved service model, recipient-number verification, business/privacy roles, DPA, Meta retention and location terms, delivery-status/webhook scope, access-token lifecycle and production-login MFA decision | 2026-08-10 / technical review; Security, Privacy and Legal acceptance pending |
