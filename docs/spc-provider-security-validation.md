...
| Google Workspace / Drive | Supplier/contact sources and logical backup artifacts | Application flows and latest-two verified logical-backup retention are documented | Partially verified | Sharing, owners, account MFA, audit, storage location, encryption assurance, deletion and restore access | Pending |
...
| Meta / WhatsApp Cloud API | Private enrolled recipient phone numbers plus login-OTP delivery and verification metadata | Feature-flagged login MFA covers every active enrolled SPC account. It uses the connected Fratelli Cosulich Hong Kong business number and approved `spc_login_mfa_code` Authentication template, requires a valid password before the six-digit WhatsApp step, and uses keyed OTP hashes, masked destinations, five-minute expiry, bounded attempts and per-user, source-IP and global send limits. All 39 active stored destinations were manually confirmed as registered WhatsApp accounts before rollout; no validation messages were sent. Missing enrollment, credential changes and delivery failures fail closed. The former ADMIN-only inactive-account test surface is retired; its append-only historical audit evidence remains ADMIN-only. | Partially verified | End-to-end all-user production evidence, recovery handling, periodic enrolled-recipient review, approved service model, business/privacy roles, DPA, Meta retention and location terms, delivery-status/webhook scope and access-token lifecycle | 2026-08-11 / technical review; Security, Privacy and Legal acceptance pending |
