...
| Evidence date | 2026-08-11 technical review; reviewer acceptance pending |
...
| Login/session protection | Database-backed repeated-failure limits, generic login errors, secure production cookies and opaque 400-day sliding SPC sessions, renewed through normal use after one day. Manual logout, revocation, expiry, account disablement and relevant password, role, phone or enrollment changes still terminate access. FCUNO logout atomically revokes every SPC session for the linked identity, and an active SPC tab revalidates when it regains focus. When the global login-MFA flag is enabled, every active enrolled SPC account must complete password validation followed by a six-digit WhatsApp code before a session is created. Missing enrollment, changed credentials or unavailable delivery fail closed. | Public authentication, per-user pre-authentication challenge and authenticated session boundaries | All-user rollout evidence, recovery ownership, threshold/policy acceptance and monitoring owner: Pending |
...
| Backups and recovery | Verified logical Drive backup process retains the latest two verified artifacts | Logical application/Drive recovery artifacts | RTO/RPO, full restore, provider backup/PITR and recovery access: Pending |
...
| Meta / WhatsApp | SPC browser-extension/company-number workflows and feature-flagged login MFA use WhatsApp. Login MFA covers every active enrolled SPC account, uses the connected Fratelli Cosulich Hong Kong business number and approved `spc_login_mfa_code` Authentication template, and requires a valid password before the six-digit code. The former ADMIN-only inactive-account test surface is retired; its append-only historical audit evidence remains restricted to ADMIN viewers. | End-to-end all-user rollout evidence; recovery process, approved service model, recipient-number verification, delivery-metadata retention, extension distribution, privacy role, DPA and provider/contract review |
...
- Audit, login-pressure, session, per-user WhatsApp login-challenge and request-correlation evidence.
