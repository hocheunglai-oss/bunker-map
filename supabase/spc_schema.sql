...
create table if not exists public.spc_enquiries (
  ...
  last_amended_by_username text,
  last_amendment_changes jsonb not null default '[]'::jsonb
...
on public.spc_group_dispatchers(active) where active;
...
begin
  ...
  insert into public.spc_lost_reason_options (audience, reason, sort_order, is_active)
  select
    normalized_audience,
...
begin
  ...
  select enquiries.*
  from public.spc_enquiries as enquiries
  where enquiries.id = reoffer_row.id;
end;
$$;
...
revoke all on function public.complete_spc_group_delivery_job(uuid, uuid, text, text, text)
  from public, anon, authenticated, service_role;
grant execute on function public.complete_spc_group_delivery_job(uuid, uuid, text, text, text)
  to service_role;
...
drop trigger if exists set_spc_lost_reason_options_updated_at
on public.spc_lost_reason_options;
create trigger set_spc_lost_reason_options_updated_at
...
drop policy if exists "spc_lost_reason_options_no_public_access"
on public.spc_lost_reason_options;
...
revoke all on table public.spc_lost_reason_options from public, anon, authenticated;
...
begin
  if to_regprocedure('public.audit_enable_table(regclass)') is not null then
    ...
    perform public.audit_enable_table('public.spc_feedback'::regclass);
  end if;
end $$;

do $$
begin
  if to_regprocedure('private.record_bunker_map_backup_mutation()') is not null then
...
-- The retired pilot account is intentionally not recreated by the canonical
-- schema. Historical audit structures below remain so prior evidence stays
-- readable until the all-user block retires the delivery-only challenge RPCs.
...
-- Allow this same canonical block to initialize an empty clean bootstrap.
-- A production migration has no such setting and therefore requires at least
-- one active, fully eligible account.
select pg_catalog.set_config('app.spc_schema_bootstrap', 'true', false);
...
-- BEGIN SPC USERNAME-FIRST LOGIN ROUTING LIMITER
create table if not exists private.spc_login_discovery_attempts (
  ...
  blocked_by text check (blocked_by in ('username', 'source_ip', 'username_and_source_ip')),
  created_at timestamptz not null default clock_timestamp(),
...
begin
  ...
  insert into private.spc_login_discovery_attempts (
    username_hash, source_ip, request_id, allowed, blocked_by, created_at
  ) values (
    p_username_hash,
    p_source_ip,
    p_request_id,
...
-- END SPC USERNAME-FIRST LOGIN ROUTING LIMITER

-- BEGIN FCUNO LINKED SINGLE LOGOUT
...
-- END FCUNO LINKED SINGLE LOGOUT

select pg_catalog.set_config('app.spc_schema_bootstrap', '', false);
