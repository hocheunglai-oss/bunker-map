...
type HealthCheckResult = Omit<HealthCheck, "id" | "label" | "checkedAt">
...
async function checkDriveBackup(): Promise<HealthCheckResult> {
  ...
  return {
    status: ageHours > BACKUP_WARNING_AGE_HOURS ? "warning" as const : "ok" as const,
...
async function checkExchangeConfig(): Promise<HealthCheckResult> {
  ...
  return {
    status: missing.length ? "warning" as const : "ok" as const,
...
async function checkCronConfig(): Promise<HealthCheckResult> {
  ...
  return {
    status: missing ? "warning" as const : "ok" as const,
