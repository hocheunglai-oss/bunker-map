...
export async function POST(request: Request) {
  try {
    ...
    if (request.headers.get("content-type")?.includes("multipart/form-data")) {
      ...
      form.append("file", file, file.name || "introduction.mp3")
    ...
    const response = await fetch("https://api.openai.com/v1/audio/speech", {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          "Speak in calm, polished British English for an international audience, including listeners who are not native English speakers.",
