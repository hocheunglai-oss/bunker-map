...
type CompactReportPayload = PublicReportPayloadByKey["compact"]

async function loadCompactReport(): Promise<CompactReportPayload | null> {
  const response = await fetch("/api/reports/compact")
  if (!response.ok) return null
  return response.json() as Promise<CompactReportPayload>
}

export default function CompactReport({ initialData }: { initialData: CompactReportPayload }) {
  const isMobile = useIsMobile()
  const [sections, setSections] = useState<ChinaReportSection[]>(initialData.snapshot?.sections ?? [])
  ...
  useEffect(() => {
    ...
    async function refreshReport() {
      const payload = await loadCompactReport()
      if (cancelled || !payload?.snapshot) return

      setReportDate(payload.snapshot.reportDate)
      setSections(payload.snapshot.sections)
  ...
  function visibleRows(section: ChinaReportSection) {
    ...
    const preview = compactExpandablePreviewRows[section.title]
      ...
      <div style={shellStyle}>
        <div style={{ ...headerCardStyle, padding: isMobile ? "16px" : "24px", marginBottom: "18px", position: "relative", overflow: "hidden" }}>
          ...
          >
            ...
            >
              <h1 style={sectionTitleStyle}>COMPACT MARKET REPORT</h1>
            </div>
            ...
            {[leftSections, rightSections].map((columnSections, columnIndex) => (
              <div key={columnIndex} style={{ display: "grid", gap: "18px", alignContent: "start" }}>
                {columnSections.map((section) => {
                  ...
                  const isExpandable = Boolean(compactExpandablePreviewRows[section.title])
