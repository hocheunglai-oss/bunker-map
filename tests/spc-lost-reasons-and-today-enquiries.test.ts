...
test("lost record enforces separate role-aware review and comments", () => {
  ...
  assert.match(lostPage, /<th>VESSEL NAME<\/th>[\s\S]*?<th>IMO<\/th>/)
  assert.match(lostPage, /enquiry\.meta\?\.imo \|\| "-"/)
  assert.match(lostPage, /const lostRecordColumnSpan = lostRecordColumnWidths\.length/)
...
test("Daily Briefing is chronological, matches IMO before vessel, and supports bulk copy", () => {
  ...
  assert.match(todayLibrary, /vessel_imo/)
  assert.match(todayLibrary, /spcVesselIdentityKeysFromValues\([\s\S]*?row\.vessel_imo,[\s\S]*?\)/)
