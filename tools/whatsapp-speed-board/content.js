(function () {
  ...
  const SEND_LOCK_KEY = "fcuno-wa-send-lock-v1"
  ...
  function runtimeUnavailableMessage() {
    return "Reload WhatsApp Web after updating the FCUNO extension."
  }
  ...
  function readSendLock() {
    ...
    const raw = document.documentElement?.getAttribute("data-fcuno-wa-send-lock") || ""
  ...
  function writeSendLock(lock) {
    ...
    document.documentElement?.setAttribute("data-fcuno-wa-send-lock", `${lock.key}|${lock.at}`)
  }
  ...
  function sanitizeEnquiry(value) {
    ...
    const body = cleanTemplateText(value.body || value.formattedText || value.notes || value.title)
    if (!body) return null
  ...
  function sanitizeSavedState(value) {
    ...
    return {
      ...
      enquiries: Array.isArray(source.enquiries)
        ? source.enquiries
            .map(sanitizeEnquiry)
            .filter(Boolean)
            .slice(0, 120)
  ...
  function isSendableEnquiry(enquiry) {
    return Boolean(enquiry && enquiryBodyText(enquiry))
  }
  ...
  function enquiryBodyText(enquiry) {
    return cleanTemplateText(enquiry?.body || enquiry?.formattedText || enquiry?.notes || enquiry?.title || "")
  }
  ...
  function prepareComposerTextForSend(text, onReady) {
    ...
    clearComposerText(composer)
    composer.focus()
    const sent = sendRuntimeMessage({ type: "fcuno-native-insert-text", text: message }, (response, runtimeError) => {
  ...
  function requestNativeClick(button) {
    ...
    return sendRuntimeMessage({ type: "fcuno-native-click", x, y }, (response, runtimeError) => {
  ...
  function requestNativeEnter(text = "", fallbackButton = null) {
    const composer = findComposer()
    composer?.focus()
    return sendRuntimeMessage({ type: "fcuno-native-enter" }, (response, runtimeError) => {
      const failed = runtimeError || !response || response.ok !== true
      window.setTimeout(() => {
        const nextComposer = findComposer()
  ...
  function renderContactList(list) {
    ...
    return `
      <section class="fcuno-wa-contact-panel" data-panel="${list}">
        <div class="fcuno-wa-contact-list" data-list="${list}">
          ${rows || `<div class="fcuno-wa-empty">No ${LIST_LABELS[list].toLowerCase()} saved.</div>`}
        </div>
      </section>
    `
  }
  ...
  function renderTemplate() {
    return `
      <section class="fcuno-wa-template" aria-label="FCUNO message template">
        <div class="fcuno-wa-template-toolbar">
          <label class="fcuno-wa-template-toggle">
  ...
  function renderEnquiries() {
    ...
    const rows = visibleEnquiries().map((enquiry) => {
      ...
      return `
        <div class="fcuno-wa-enquiry${isNew ? " is-new" : ""}${isDragging ? " is-dragging" : ""}" draggable="true" data-action="seen-enquiry" data-id="${escapeHtml(enquiry.id)}">
    ...
    return `
      <section class="fcuno-wa-enquiry-panel">
        <div class="fcuno-wa-enquiry-actions">
        ...
        ${renderTemplate()}
        <div class="fcuno-wa-enquiry-list">
          ${state.enquiryError ? `<div class="fcuno-wa-error">${escapeHtml(state.enquiryError)}</div>` : ""}
          ${rows || `<div class="fcuno-wa-empty">No enquiries sent from Enquiry Worksheet yet.</div>`}
        </div>
      </section>
    `
  }
  ...
  function renderCrudeWatch() {
    ...
    return `
      <div class="fcuno-wa-crude" aria-label="${escapeHtml(sourceLabel || "Verified ICE Brent unavailable")}" title="${escapeHtml(sourceLabel || state.crudeError || "Verified ICE Brent unavailable")}">
    ...
    host.innerHTML = `
      <div class="fcuno-wa-shell${state.collapsed ? " is-collapsed" : ""}">
        <div class="fcuno-wa-head">
          ...
          ${renderCrudeWatch()}
          <button class="fcuno-wa-icon" type="button" data-action="toggle">${state.collapsed ? "‹" : "›"}</button>
        </div>
        <div class="fcuno-wa-main">
          <div class="fcuno-wa-contacts">
  ...
  function clearDropMarkers(root = document) {
    root.querySelectorAll(".fcuno-wa-row.is-drop-before, .fcuno-wa-row.is-drop-after, .fcuno-wa-row.is-send-target").forEach((row) => {
  ...
  function setContactDragData(event, id) {
    ...
    event.dataTransfer.setData("application/x-fcuno-contact-id", id)
  ...
  function setEnquiryDragData(event, ids) {
    ...
    event.dataTransfer.setData("application/x-fcuno-enquiry-id", enquiryIds[0])
    event.dataTransfer.setData("application/x-fcuno-enquiry-ids", JSON.stringify(enquiryIds))
  ...
  function enquiryDragIds(event) {
    ...
    const rawIds = event.dataTransfer.getData("application/x-fcuno-enquiry-ids")
    if (rawIds) {
      try {
    ...
    const id = event.dataTransfer.getData("application/x-fcuno-enquiry-id") || state.dragging || ""
    return id ? [id] : []
  }
  ...
  function contactDragId(event) {
    ...
    return (
      event.dataTransfer.getData("application/x-fcuno-contact-id") ||
  ...
  function clearEnquiryDragMarkers(root = document) {
    root.querySelectorAll(".fcuno-wa-enquiry.is-dragging").forEach((row) => {
  ...
  function markDraggingEnquiries(root, ids) {
    clearEnquiryDragMarkers(root)
    const idSet = new Set((ids || []).map(cleanText).filter(Boolean))
    root.querySelectorAll(".fcuno-wa-enquiry[data-id]").forEach((row) => {
  ...
  function setEnquiryDragImage(event, row, ids) {
    ...
    preview.className = "fcuno-wa-drag-preview"
    const idSet = new Set(ids.map(cleanText).filter(Boolean))
    visibleEnquiries()
  ...
  function bindEvents(host) {
    ...
    host.querySelectorAll("[data-action='contact-menu']").forEach((button) => {
      ...
      button.addEventListener("dragstart", (event) => {
        const row = button.closest(".fcuno-wa-row")
        if (!row) return
    ...
    host.querySelectorAll(".fcuno-wa-row-actions").forEach((zone) => {
      const row = zone.closest(".fcuno-wa-row")
    ...
    host.querySelectorAll(".fcuno-wa-row").forEach((row) => {
      row.addEventListener("click", (event) => {
        if (event.target instanceof Element && event.target.closest(".fcuno-wa-row-actions")) return
        cancelContactMenuAutoHide()
        state.contactMenuId = ""
      ...
      row.addEventListener("dragover", (event) => {
        event.preventDefault()
        event.stopPropagation()
        if (dragHasType(event, "application/x-fcuno-enquiry-id") || state.draggingType === "enquiry") {
    ...
    host.querySelectorAll(".fcuno-wa-contact-list").forEach((list) => {
    ...
    host.querySelectorAll(".fcuno-wa-enquiry[draggable='true']").forEach((row) => {
      row.addEventListener("dragstart", (event) => {
        const id = row.dataset.id || ""
  ...
  if (typeof window !== "undefined" && window.__FCUNO_WA_ENABLE_TEST_API__) {
    window.__FCUNO_WA_TEST_API__ = {
      state,
