...
Worker release `fcuno-exchange-runbook/2026-09-02.20` runs in the dedicated Azure Automation `FCUNO-PowerShell-7-6` environment with PowerShell 7.6 and ExchangeOnlineManagement 3.10.1; module selection uses the runtime's major and minor version so Azure's prerelease label cannot select an older incompatible module.
