...
Worker release `fcuno-exchange-runbook/2026-07-24.1` also certifies every managed distribution group's exact lowercase primary SMTP address. Azure Automation variable `EXCHANGE_ADDRESSBOOK_DOMAIN` must be `cosulich1.onmicrosoft.com`; the worker and database certification both fail closed when the address is absent, differs from the FCUNO alias, or uses another domain. Full reconciliations hold a 180-minute mutation lease so a slow Exchange Online command cannot expose a second writer; incremental runs retain the normal 30-minute lease. Outlook Templates resolve managed recipients only from the latest settled certified projection, so stale, missing, or ambiguous recipient evidence cannot be inserted.
