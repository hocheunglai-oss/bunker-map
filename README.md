...
Worker release `fcuno-exchange-runbook/2026-07-23.3` also certifies every managed distribution group's exact lowercase primary SMTP address. Azure Automation variable `EXCHANGE_ADDRESSBOOK_DOMAIN` must be `cosulich1.onmicrosoft.com`; the worker and database certification both fail closed when the address is absent, differs from the FCUNO alias, or uses another domain. Outlook Templates resolve managed recipients only from the latest settled certified projection, so stale, missing, or ambiguous recipient evidence cannot be inserted.
